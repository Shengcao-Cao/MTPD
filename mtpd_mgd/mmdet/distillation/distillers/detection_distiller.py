...
class DetectionDistiller(BaseDetector):
    ...
    def __init__(self,
        ...
        if init_student:
            print('Inheriting teacher weights from', teacher_pretrained)
            t_checkpoint = _load_checkpoint(teacher_pretrained)
            all_name = []
            # inherit from first stage of cascade
            if 'cascade' in teacher_pretrained:
                print('Inheriting from first stage of cascade')
                for name, v in t_checkpoint['state_dict'].items():
                    if name.startswith('backbone.'):
                        continue
                    if name.startswith('roi_head'):
                        if name.startswith('roi_head.bbox_head.0.'):
                            name = name.replace('roi_head.bbox_head.0.', 'roi_head.bbox_head.')
                            # hard-coded number of classes (for COCO), change if necessary
                            if name == 'roi_head.bbox_head.fc_reg.weight':
                                v = v.repeat(80, 1)
                            if name == 'roi_head.bbox_head.fc_reg.bias':
                                v = v.repeat(80)
                            all_name.append((name, v))
                        elif name.startswith('roi_head.mask_head.0.'):
                            name = name.replace('roi_head.mask_head.0.', 'roi_head.mask_head.')
                            all_name.append((name, v))
                    else:
                        all_name.append((name, v))
            else:
    ...
    def init_weights_student(self, path=None):
        """Load the pretrained model in student detector.
